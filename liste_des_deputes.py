...
#nettoyer le texte du code source, enlever les balises et la caractères spéciaux
...
regex_metier = r'(?<=Parcours professionnel).*?(?=Parcours politique)'
regex_region = r'(?<=\,).*'
...
for link in dep_link:
    ...
    try:
        metier.append(re.findall( regex_metier ,get_text_html(link))[0].strip())
    except: 
        metier.append("")


#Consolider toutes les information collectées dans un seul dataframe 
...
#récupérer le rang en chiffre des députés     
